...
namespace DoMesCenter
{
  ...
  public sealed partial class MainPage : Page
  {
    ...
    private TeleInformation _teleInfo = null;
    private DoMesCenterCsv _fichierCsv = new DoMesCenterCsv();
    private bool _iDepasse = false;
    ...
    private void InitTeleInformation()
    {
      //ParametresPortSerie parametres = new ParametresPortSerie("FT230X Basic UART", @"\\?\FTDIBUS#VID_0403+PID_6015+DA00OGRPA#", 1200, SerialParity.Even, SerialStopBitCount.One, 7, SerialHandshake.None, 800, 0);     
      ParametresPortSerie parametres = new ParametresPortSerie("", @"\\?\ACPI#BCM2837#4#", 1200, SerialParity.Even, SerialStopBitCount.One, 7, SerialHandshake.None, 800, 0);  // Pi2
      //ParametresPortSerie parametres = new ParametresPortSerie("", @"\\?\ACPI#BCM2836#0#", 1200, SerialParity.Even, SerialStopBitCount.One, 7, SerialHandshake.None, 800, 0);   // Pi3
      _teleInfo = new TeleInformation(parametres, true, 160);
      TeleInfoSPL.DataContext = new TeleInformationVM(_teleInfo);
      _teleInfo.SurNouvellesDonnees += _teleInfo_SurNouvellesDonnees;

      PeripheriqueSerie.PeripheriquesSerie.DemarrerSurveillance();
    }
    private async void LireValeurs(DateTime? instant)
    {
      ...
      try
      {
        ...
        if (instant.HasValue)
          _fichierCsv.AjouterMesures(instant.Value, DHT22Result?.Temperature, TMP007res?.TemperatureAmbiante, TMP007res?.TemperatureIR, DHT22Result?.Humidite, HDC1000res?.Humidite, BMP280res?.Pression, OPT3001res?.Luminosite, _teleInfo.IndexHP_BASE, _teleInfo.IndexHC, _teleInfo.PeriodeTarifaireEnCours == "HC..", _iDepasse);
        _iDepasse = false;
      }
      finally
      {
